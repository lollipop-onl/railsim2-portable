...
DirectInput8 / Win32 cursor APIs          lib/input.cpp (poll thread + Scan*)
   ...
   svi.key/btn/wheel/joy buffers  ----->  GetKey / GetButton / GetWheel / GetCursor* / GetJoy
                                                    |
                    game + lib callers (CCamera, UI widgets, modes, editbox, Åc)
```
...
1. Background thread (`InputPollingThread`) calls `InputPollOnce()` Å® `GetDeviceState` on keyboard/mouse.
2. Main thread `ScanInputDevice()` merges poll buffers, then `ScanKeyboard` / `ScanMouse` / `ScanJoyStick`.
3. `GetKey` / `GetButton` / `GetJoy` expose edge/hold semantics (`S_FREE` Åc `S_HOLD`).
...
| `IDirectInputDevice8::EnumObjects` | `InitJoyStick` | `DIDFT_AXIS` Å® `EnumAxisCallback` |
...
| `CCursor.cpp` | `CCursor::ScanInput` (also calls `FixCursor` Å® `SetCursor` center) |
...
|-------|------|-----------|
| Poll | `lib/input.cpp` `InputPollOnce` | `DIMOUSESTATE.lZ` Å® `svi.wheelPoll` |
| Frame | `lib/input.cpp` `ScanMouse` | `svi.wheel` Å© accumulated poll |
...
### Relative movement (`CCursor` Å® camera / scenery)
...
| `CCamera::Slide` | `CCamera.cpp` | pan/slide from delta Å~ distance Å~ mode |
...
|----------|---------------------------|-------|
| `GetKey(int id)` | `svi.key[]` indexed by **`DIK_*` scan code** | Returns `S_FREE`Åc`S_HOLD` |
...
| `GetJoy(n, id)` | `svi.joy[][]`, **`DIJ_*` index** | axis thresholds Å}500 in `ScanJoyStick` |
...
|--------|-----------|---------|
| `DIM_*` | `LEFT=0`, `RIGHT=1`, `MIDDLE=2` | UI widgets, `CCamera`, `CSceneryMode`, Åc |
| `DIJ_*` | `UP`Åc`BOTTOM` (0?5), `BT1`Åc`BT8` (6?13) | `ScanJoyStick` mapping; debug `ShowJoyStickInfo` only |
...
| `DIK_0` Åc `DIK_4` | yes | `CSceneryMode` scene slots (+ numpad twins) |
| **`DIK_5` Åc `DIK_8`** | **no** | `CGameMode` menu icons (`DIK_1+j-1`, `MODE_SUB` up to 8) |
...
| `DIK_NUMPAD0` Åc `DIK_NUMPAD4` | yes | scenery scene shortcuts |
...
Non-M3 but same API: widget `ScanInput` methods (`CPushButton`, `CListView`, Åc) ? needed for editor UI, not for bare 3D viewport smoke.
