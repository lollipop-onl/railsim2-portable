...
- **SDL2 (`port/rs2_msg_sdl.cpp`, `RS2_HAVE_SDL2`)**: `SDL_QUIT` becomes `WM_CLOSE`; focus gained / lost becomes `WM_ACTIVATEAPP(TRUE/FALSE)`; `SIZE_CHANGED` updates the client size and posts `WM_SIZE`. Events are taken one type at a time (`SDL_QUIT`, `SDL_WINDOWEVENT`), so keyboard, mouse and wheel events stay queued for `rs2_input_sdl.cpp` (`drain_wheel`). Until `CreateWindowEx` has run the pump drains nothing, so an early `SDL_QUIT` waits in SDL's queue instead of being dropped. Wait is a 10 ms `SDL_Delay` + pump, not `SDL_WaitEvent`, because those leftover input events would make `SDL_WaitEvent` return at once.
