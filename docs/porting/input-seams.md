...
## Win32 message queue (`port/rs2_msg.cpp`)

- **Issue**: [#205](https://github.com/lollipop-onl/railsim2-portable/issues/205) (parent [#8](https://github.com/lollipop-onl/railsim2-portable/issues/8))
- **Why it exists**: `CGameMode::Spin`, `Opening` and the `lib/main.cpp` loop run `while(PeekAllMessage()){ if(IsActive()) ... else WaitMessage(); }`. `svw.fActive` only turns TRUE when `MessageProc` sees `WM_ACTIVATEAPP`, and `Exit()` / `SendWM_CLOSE` only end the loop when `WM_CLOSE` reaches `MessageProc`, which calls `PostQuitMessage`. With no-op stubs neither happened.
- **Surface**: `RegisterClassEx` keeps the `WndProc`; `CreateWindowEx` gives each window a distinct handle (never `GetDesktopWindow()`'s `(HWND)1`), remembers its client size and posts `WM_ACTIVATEAPP(TRUE)`. `PeekMessage` / `GetMessage` / `DispatchMessage` / `PostMessage` / `SendMessage` / `PostQuitMessage` / `GetClientRect` / `DestroyWindow` / `WaitMessage` work on one process-wide FIFO. `WM_QUIT` is a flag handed out once the posted messages ahead of it are gone, as on Windows. `Sleep` is `std::this_thread::sleep_for`. `TranslateMessage` and `DefWindowProc` stay no-ops: `WM_CHAR` comes from the IME seam, not from key messages.
- **Backend hooks**: `rs2_msg_backend_pump` (every `PeekMessage` / `GetMessage` / `WaitMessage`) and `rs2_msg_backend_wait` (empty queue). The check stub pumps nothing and its wait reports that nothing can arrive, so `GetMessage` on an empty queue returns -1 instead of hanging.
- **SDL2 (`port/rs2_msg_sdl.cpp`, `RS2_HAVE_SDL2`)**: `SDL_QUIT` becomes `WM_CLOSE`; focus gained / lost becomes `WM_ACTIVATEAPP(TRUE/FALSE)`; `SIZE_CHANGED` updates the client size and posts `WM_SIZE`. Events are taken one type at a time (`SDL_QUIT`, `SDL_WINDOWEVENT`), so keyboard, mouse and wheel events are left to `rs2_input_sdl.cpp`: each poll reads the state, sums the wheel (`drain_wheel`) and flushes the other key, text and mouse events, so SDL's 65535-event queue never fills and drops `SDL_QUIT` ([#212](https://github.com/lollipop-onl/railsim2-portable/issues/212)). While the game is inactive nothing polls, so `rs2_msg_backend_wait` flushes key, text, mouse and wheel events itself; a background wheel is dropped as DirectInput would, not replayed on the first active frame. Until `CreateWindowEx` has run the pump drains nothing, so an early `SDL_QUIT` waits in SDL's queue instead of being dropped. Wait is a 10 ms `SDL_Delay` + pump, not `SDL_WaitEvent`, because any input event arriving during the wait would make `SDL_WaitEvent` return at once.
- **Test**: `rs2_msg_self_test` (stub backend, no SDL).
