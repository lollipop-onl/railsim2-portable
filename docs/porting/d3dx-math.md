...
- **Degenerate input** follows D3DX8, not a safer guess: `Vec3TransformCoord` divides by w even when it is 0, `MatrixInverse` returns NULL for a singular matrix and leaves the output untouched, and `MatrixShadow` normalizes the plane first.
