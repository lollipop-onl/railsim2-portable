...
| Missing GDI / Win32 UI types | `CPixelbit.cpp` (`BITMAPFILEHEADER`, `ReadFile`), `lib/font.cpp` (`LOGFONT`, `DT_*`), `lib/texture.cpp`, `lib/sprite.cpp` (`::SetRect` not in stub). `CPixelbitStamp.cpp` (`SetDIBitsToDevice`, `SRCCOPY`) sat here too (closed by `#178`) |
...
and `SRCCOPY`, the first GDI row to go.
